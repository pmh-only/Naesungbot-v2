  ...
  play: (msg, command) => {
    function play (url) {
      ...
      msg.member.voice.channel.join().then(connection => {
        const streamOptions = { seek: 0, volume: 1, bitrate: 64000 }
    ...
    function searchNoPlay () {
      const youtube = stringhandler.cutTextHead('play ', command)
      const link = 'https://www.youtube.com/results?search_query=' + encodeURI(youtube)
        ...
        if (error) msg.reply(error)
          ...
          msg.member.voice.channel.join().then(connection => {
            ...
            https.get(body.download_url + '?client_id=aGG0HvvLMMohiFsNq85gvca9MB6wAxHP', (response) => {
              ...
              const outStream = fs.createWriteStream('./sc/' + body.id + '.mp3')
              response.pipe(outStream)
              const write = fs.createWriteStream('./sc/' + body.id)
              write.on('end', () => {
                ffmpeg('./sc/' + body.id)
                  .inputFormat('mp3')
